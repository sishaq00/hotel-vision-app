...
export interface HotelSettings {
  ...
  // LAN sync
  syncServerUrl?: string;      // e.g. "http://192.168.1.10:8090" — PocketBase on local network
...
interface HotelState {
  setupComplete: boolean;          // true once onboarding wizard is finished
  rooms: Room[];
      ...
      return {
        ...
        checkOut: (id, opts) => {
          ...
          if (outstanding > 0 && !opts?.markPaid && !opts?.force) {
            // Return sentinel object so callers can show a meaningful error.
            // null is kept for "reservation not found / wrong status" cases.
            return { __balanceError: true, outstanding } as unknown as InvoiceSnapshot;
          }
