    ...
    <AppLayout title={t("nav.payments")} subtitle={t("sub.payments")}>
      <div className="space-y-6">
        <div className="grid grid-cols-1 gap-3 sm:grid-cols-3">
          {[
            { label: "Total collected", value: `${settings.currency} ${totals.paid.toLocaleString(undefined,{minimumFractionDigits:2,maximumFractionDigits:2})}`, accent: "border-l-4 border-l-emerald-500", sub: "All time paid" },
            { label: "Pending",         value: `${settings.currency} ${totals.pending.toLocaleString(undefined,{minimumFractionDigits:2,maximumFractionDigits:2})}`, accent: "border-l-4 border-l-amber-500",  sub: "Awaiting settlement" },
            { label: "Transactions",    value: payments.length,                                                                       accent: "border-l-4 border-l-blue-500",    sub: "Total records" },
          ].map(({ label, value, accent, sub }) => (
            <div key={label} className={`relative overflow-hidden rounded-xl border border-border/60 bg-card p-4 shadow-sm ${accent}`}>
