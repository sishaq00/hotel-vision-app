...
function UsersInner() {
  ...
  return (
    <div className="space-y-6">
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="border-b border-border/60 bg-muted/30">
              ...
              <th className="px-4 py-2.5 text-left text-[11px] font-semibold uppercase tracking-wider text-muted-foreground">Permissions</th>
          ...
          <tbody className="divide-y divide-border">
            {users.map((u) => (
              <tr key={u.id} className="group border-b border-border/40 transition-colors hover:bg-muted/30 last:border-0">
                <td className="px-4 py-3">
                  <div className="flex items-center gap-2">
                    <div className={`flex h-8 w-8 items-center justify-center rounded-full text-xs font-bold ${u.role === "admin" ? "bg-primary text-primary-foreground" : "bg-primary/10 text-primary"}`}>
                      {u.role === "admin" ? <ShieldCheck className="h-4 w-4" /> : u.fullName.slice(0, 2).toUpperCase()}
                    </div>
                    <div>
      ...
      <div className="rounded-xl border border-border/60 bg-amber-50/50 p-4 dark:bg-amber-950/10">
        <p className="text-xs text-muted-foreground">
          <strong className="text-amber-700 dark:text-amber-400">🔐 Offline security note:</strong> User credentials are stored locally on this device using PBKDF2 hashing. Lock your device when unattended and export regular backups from Settings → Backup.
        </p>
      </div>
