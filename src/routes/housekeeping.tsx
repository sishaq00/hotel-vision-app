...
function HousekeepingPage() {
  ...
  return (
    <AppLayout
      title="Housekeeping"
      subtitle={`${counts.dirty + counts.departure + counts.stayover} rooms need cleaning · ${counts.assigned} assigned`}
    >
      <div className="space-y-4">
      {/* ── KPI bar ────────────────────────────────────────────────────── */}
      <div className="grid grid-cols-3 gap-3 sm:grid-cols-6">
        {[
          { label: "Dirty",     value: counts.dirty,     bar: "bg-amber-500",   bg: "border-l-amber-500"   },
          { label: "Departure", value: counts.departure, bar: "bg-orange-500",  bg: "border-l-orange-500"  },
          { label: "Stayover",  value: counts.stayover,  bar: "bg-blue-500",    bg: "border-l-blue-500"    },
          { label: "Clean",     value: counts.clean,     bar: "bg-emerald-500", bg: "border-l-emerald-500" },
          { label: "Inspected", value: counts.inspected, bar: "bg-violet-500",  bg: "border-l-violet-500"  },
          { label: "OOO",       value: counts.ooo,       bar: "bg-rose-500",    bg: "border-l-rose-500"    },
        ].map(({ label, value, bg }) => (
          <div key={label} className={`relative overflow-hidden rounded-xl border border-border/60 bg-card p-3 shadow-sm border-l-4 ${bg}`}>
