    ...
    <AppLayout title={t("nav.guests")} subtitle={t("sub.guests")}>
      <div className="space-y-4">
        ...
        <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
          ...
          <StatCard label="VIP guests"    value={stats.vip}   accent="amber" />
...
function StatCard({
  label, value, accent,
}: { label: string; value: number; accent?: "amber" | "red" | "blue" }) {
  const bar = accent === "amber" ? "border-l-amber-500"
    : accent === "red"   ? "border-l-rose-500"
    : accent === "blue"  ? "border-l-blue-500"
    : "border-l-slate-400";
  const val = accent === "amber" ? "text-amber-600"
    : accent === "red" ? "text-rose-600" : "text-foreground";
  return (
    <div className={`relative overflow-hidden rounded-xl border border-border/60 bg-card p-4 shadow-sm border-l-4 ${bar}`}>
      <p className={`text-2xl font-semibold ${val}`}>{value}</p>
