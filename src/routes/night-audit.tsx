          ...
          {step === 0 && (
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
                <Stat label="Arrivals"  value={summary.arrivals.length}  accent="border-l-violet-500" />
                <Stat label="In-house"  value={summary.inHouse.length}   accent="border-l-blue-500"   />
                <Stat label="Departures"value={summary.departures.length} accent="border-l-amber-500" />
                <Stat label="Occupancy" value={`${summary.occupancy.toFixed(0)}%`}
                  hint={`${summary.occupiedRooms}/${summary.totalRooms}`} accent="border-l-emerald-500" />
                <Stat label="Revenue" value={`${settings.currency} ${summary.revenue.toFixed(2)}`}
                  hint={`${summary.paymentCount} payment(s)`} wide accent="border-l-green-500" />
                <Stat label="Pending no-shows" value={summary.noShows.length} wide
                  accent={summary.noShows.length > 0 ? "border-l-rose-500" : "border-l-slate-400"} />
              </div>
...
function Stat({ label, value, hint, wide, accent }: { label: string; value: string | number; hint?: string; wide?: boolean; accent?: string }) {
  return (
    <div className={cn(
      "relative overflow-hidden rounded-xl border border-border/60 bg-card p-4 shadow-sm",
      wide && "sm:col-span-2",
...
function Row({ k, v }: { k: string; v: string }) {
  return (
    <div className="flex items-center justify-between rounded-lg py-2 px-2 odd:bg-muted/20">
