...
BEGIN
  ...
  INSERT INTO reservations(guest_id,room_id,check_in,check_out,status,total_amount,source,confirmation_number)
  VALUES (v_guest, v_room, CURRENT_DATE, CURRENT_DATE+2, 'confirmed', 200, 'walk-in',
  ...
  INSERT INTO folio_charges(folio_id,description,amount,category,posted_by)
    VALUES (v_folio,'Room night 1',100,'room',p_actor),
           (v_folio,'Room night 2',100,'room',p_actor),
           (v_folio,'Minibar',15,'fnb',p_actor);
  ...
  v_log := v_log || jsonb_build_object('step','8_complete_housekeeping','final_status','inspected');
